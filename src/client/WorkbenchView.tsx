...
const FISH_SVG = (
  <svg viewBox="0 0 23.16 17.04" width={72} height={53} fill="#679EFE" aria-hidden="true">
...
export function WorkbenchView({ t }: WorkbenchViewProps) {
  const [config, setConfig] = useState<UiCustomizerConfig>(() => loadConfig())

  useEffect(() => {
    const onChange = (): void => setConfig(loadConfig())
  ...
  return (
    <div style={{
      ...
      justifyContent: 'center', gap: 22, padding: 40, textAlign: 'center',
    }}>
      <div style={{ opacity: 0.9 }}>
        {config.enabled && config.logoUrl !== ''
          ? <img src={config.logoUrl} alt="" style={{ width: 88, height: 88, objectFit: 'contain' }} draggable={false} />
          : FISH_SVG}
      </div>
      <div style={{ fontSize: 34, fontWeight: 800, lineHeight: 1.3 }}>
        {config.enabled && config.heroText !== '' ? config.heroText : 'Workbench'}
      </div>
      <div style={{ fontSize: 15, opacity: 0.6 }}>{t('workbenchIntro')}</div>
            ...
            {links.map((link, index) => (
              <a key={index} href={link.url} target="_blank" rel="noreferrer"
                style={{
                  ...
                  border: '1px solid rgba(103,158,254,0.4)', background: 'rgba(65,118,230,0.1)',

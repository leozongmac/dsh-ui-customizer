...
export const DEFAULT_CONFIG: UiCustomizerConfig = {
  enabled: true,
...
function isLink(value: unknown): value is WorkbenchLink {
  return typeof value === 'object' && value !== null
    && typeof (value as WorkbenchLink).label === 'string'
    && typeof (value as WorkbenchLink).url === 'string'
}

/** Merge whatever survived in localStorage onto the defaults (never throw). */
export function loadConfig(): UiCustomizerConfig {
  const base = { ...DEFAULT_CONFIG, links: DEFAULT_CONFIG.links.map(l => ({ ...l })) }
  try {
    const raw = window.localStorage.getItem(STORAGE_KEY)
    if (raw === null) return base
    const parsed = JSON.parse(raw) as Partial<UiCustomizerConfig>
...
export function saveConfig(config: UiCustomizerConfig): void {
  try {
